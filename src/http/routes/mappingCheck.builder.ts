...
function buildFullConditions(robot: VirtualRobot): MappingCheckDataPayload {
  ...
  const fixture = readMappingCheckFixture();
  return {
    ...fixture,
...
function readMappingCheckFixture(): MappingCheckDataPayload {
  return fixtureLoader.read('mapping/check_conditions.jsonc', raw => {
    if (typeof raw !== 'object' || raw === null) {
      throw new Error('fixtures/mapping/check_conditions.jsonc must contain an object');
    }
    const candidate = raw as Partial<Record<keyof MappingCheckDataPayload, unknown>>;
    const required: readonly (keyof MappingCheckDataPayload)[] = [
      'bluetooth_status',
      'bluetooth_msg',
      'cellular',
      'wifi',
    ...
    for (const key of required) {
      if (typeof candidate[key] !== 'string') {
        throw new Error(`fixtures/mapping/check_conditions.jsonc field ${key} must be a string`);
      }
    }
    return candidate as MappingCheckDataPayload;
  });
}
