...
- `400`：缺少 `Authorization` / `platform`、未知 platform
- `401`：token 校验失败
- `503`：业务不可用
...
- 握手成功后服务端立即建立登录态，**无** WS 内 login 指令
...
| `sub_status` | string | N | 主状态内阶段；`none` 或空表示无有效子状态（App 会回退读 legacy `phase`） |
| `work_msg` | string | N | `work_status` 补充说明（如故障原因） |
| `battery` | object | N | 电池状态，见 [battery](#data-battery) |
| `signals` | object | N | 网络信号，见 [signals](#data-signals) |
...
路径前缀：`/ratel/api/v1/mapping/`（自检为 `/ratel/api/v1/robot/self_check`）。
...
用于 `list.task_notify` 及 `NOTIFY_MOW_STATUS.data.payload`。
...
| 开始 / 暂停 / 恢复 / 停止 / 切换模式 | `postStartMapping` 等 | HTTP，失败 BLE 回退 |
...
| `LOCATION_REGISTER` | `useLocationRegistration` → `wsSend`；成功后 30s 内无 `ROBOT_LOCATION` 打 WARN `location.register.no_robot_location` |
