...
| `POST` | `/ratel/api/v1/mapping/expansion` | `{ code: 200, message: 'SUCCESS' }` — 发起地图扩展（APP端接口文档 §9.1），**地图编辑页**「添加 → 添加草坪」入口；body `{ sn, map_id }`。成功响应**只有 `code`/`message`，不带 `data`**；`data: { robot_code, robot_message }` 仅在设备拒绝执行时出现。§9.1 规定 PuduLink 只按 `sn` 取设备 MAC 后透传 `RATEL_MAPPING_TASK_EXPANSION`，不校验 `map_id` 是否存在、不写数据库，故 mock 同样不做地图存在性校验。接口成功仅代表设备已确认指令：mock 先同步等待 `MAPPING_EXPANSION_ACK_DELAY_MS` 模拟「等待设备回包」，响应后再异步推 `precondition → leave_dock → find_boundary`（`mode=remote`），全部落在 Mower `START_STATUS_WATCHDOG_MS`（12s）之内（延迟常量见 `SimulatorDefaults.ts`）。成功时切到 `EXPAND_AREA_DATASET`（`mapping_lawn2_aisle`），并**保留既有 `mappingLabels`**（新通道 label 追加而非取代）。错误：`400` 缺 `sn`/`map_id`（下发前即拒绝，不走等待延迟）、`404`「无法获取设备 MAC」（`sn` 与模拟器不符）、`409` 设备忙（建图/割草任务处于 `ON_THE_WAY`/`PAUSE`）或草坪数已达 `EXPAND_AREA_MAX_LAWNS`。**[需后端确认]** §9.1 声明 PuduLink 不写数据库，但 Mower 的 `confirmLaunch → recoverTaskId` 需要 `ratel_mapping_task/list` 中存在活跃任务，因此 mock 侧仍创建一条 `mode=remote`、`map_id` 取请求值的任务记录——真机上这条任务应由设备自行上报。 |
