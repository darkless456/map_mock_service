...
- **`ratel_mapping_task/create` 新增 `mode:'extend'`（扩展建图，APP端接口文档 v9）——支撑地图编辑页「添加 → 添加草坪」。** 取代本仓库此前（同为未发布）实现的专用端点 `POST /ratel/api/v1/mapping/expansion`，该路由已删除且**不留兼容别名**（旧路径落 404，有回归用例守着）。行为：复用请求里的既有 `map_id`（不校验其存在性）、任务记录 `mode` 记为 `extend`（任务列表回给 App，App 侧 `normalizeSessionMode` 归一为手摇会话 `remote`）、**刻意保留既有 `mappingLabels`**（地图资产而非会话状态，`useMappingPassageCapture` 靠它做通道端点归属）；与普通 create 一样**必须返回 `task_id`**（App 对 extend 同样 fail-fast，缺 task_id 直接拒绝进入建图页）。响应后由 `VirtualRobot.startExtendMapping` 异步推 `precondition → leave_dock → find_boundary(mode=remote)`，两个延迟常量（`MAPPING_EXTEND_*_DELAY_MS`）都远小于 Mower 的 `START_STATUS_WATCHDOG_MS`（12s）。成功时切到 `EXPAND_AREA_DATASET`（`mapping_lawn2_aisle`）。错误：`400` 缺参、`404`「无法获取设备 MAC」、`409` 设备忙 / 草坪数达 `EXPAND_AREA_MAX_LAWNS`。注意与 `ratel_mapping_task/action` 的 `EXPAND_AREA` 仍是**两条不同入口**：后者属于建图完成页、要求 `sub_status === 'expand_area'`，不能复用。
- **新增场景 `mapedit_add_lawn`**：建完一块草坪后 `idle` 收口，停在「设备空闲 + 已有地图」这一进入地图编辑页的真实前置，等 App 点「添加草坪」发起扩展建图（`create` + `mode:'extend'`）；后续状态推进全部由 `VirtualRobot` 定时器驱动。
- **新增 `src/http/routes/mappingErrors.ts`**：`MAPPING_ERROR_STATUS` 从 `mappingTask.routes.ts` 提取为共享映射，`ratel_mapping_task/action` 与 `ratel_mapping_task/create` 共用同一套 `400/404/409/422` 语义。
